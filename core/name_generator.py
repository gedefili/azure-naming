...
def _template_context(region: str, environment: str, slug: str, optional_inputs: Mapping[str, str]) -> Mapping[str, str]:
...
def build_name(region, environment, slug, rule, optional_inputs):
    ...
    if template:
        context = _normalise_context(_template_context(region, environment, slug, optional_inputs))
        try:
