"""Custom naming rule provider enforcing US-only regions and strict segments."""

from __future__ import annotations

from typing import Mapping

from utils import naming_rules
from utils.naming_rules import DisplayField, DictionaryRuleProvider, NamingRule
...
class USStrictRuleProvider:
    ...
    def _validate_storage_payload(self, payload: Mapping[str, object]) -> None:
        ...
        if region not in _ALLOWED_REGIONS:
            raise ValueError(
                f"region must be one of {sorted(_ALLOWED_REGIONS)}"
            )
...
def get_provider() -> USStrictRuleProvider:
    """Factory used by the NAMING_RULE_PROVIDER environment setting."""
